...
<Callout type="info">
**端口默认只监听 `127.0.0.1`。** `docker-compose.selfhost.yml` 把每个 publish 出来的端口默认绑到 loopback —— `ss -tlnp` 不会看到 `0.0.0.0:8080`，外网/其它机器默认根本连不上。可信内网测试可以在 `.env` 里设置 `MULTICA_BIND_ADDRESS=0.0.0.0`，但服务密钥和 Postgres 凭据绝不能直接暴露到公网。公网或跨机访问请优先用反向代理在前面终结 TLS，详见下方 [Step 5b —— 跨机访问：用反向代理把服务挡在前面](#5b-跨机访问用反向代理把服务挡在前面)。
</Callout>
...
因为 compose 默认监听 `127.0.0.1`，从别的机器跑的 daemon 是连不上 `http://<server-ip>:8080` 的——通常也不应该直接连，否则服务密钥可能暴露在公网。可信内网测试可以在 `.env` 里设置 `MULTICA_BIND_ADDRESS=0.0.0.0` 并重启 stack。公网部署的正确做法是在 server 上跑一个反向代理（Caddy / nginx / Cloudflare Tunnel），由它终结 TLS，再反代到 `127.0.0.1:8080`（backend）和 `127.0.0.1:3000`（frontend）。然后把 CLI 指到公开的 HTTPS 域名：
