...
<Callout type="info">
**포트는 기본적으로 `127.0.0.1`에서만 수신합니다.** `docker-compose.selfhost.yml`은 공개된 모든 포트를 기본적으로 loopback에 바인딩합니다 — `ss -tlnp`에서는 `0.0.0.0:8080`이 보이지 않으며, 다른 기기에서는 기본적으로 서비스에 접근할 수 없습니다. 신뢰할 수 있는 LAN 전용 테스트에서는 `.env`에 `MULTICA_BIND_ADDRESS=0.0.0.0`을 설정할 수 있지만, 서버 시크릿과 Postgres 자격 증명이 공개 인터넷에 노출되어서는 절대 안 됩니다. 공개 또는 기기 간 접근이 필요하면 TLS를 종료하는 리버스 프록시를 스택 앞에 두세요 — [5b단계 — 기기 간: 리버스 프록시를 앞에 두기](#5b-cross-machine-front-with-a-reverse-proxy)를 참고하세요.
</Callout>
...
compose 스택은 기본적으로 `127.0.0.1`에서 수신하므로, 다른 기기에 있는 데몬은 `http://<server-ip>:8080`에 직접 연결할 수 없습니다 — 보통 그렇게 직접 연결하는 것도 원하지 않습니다. 그렇지 않으면 서버 시크릿이 공개 인터넷에서 접근 가능해질 수 있기 때문입니다. 신뢰할 수 있는 LAN 전용 테스트에서는 `.env`에 `MULTICA_BIND_ADDRESS=0.0.0.0`을 설정하고 스택을 재시작할 수 있습니다. 공개 배포에서는 서버에 TLS를 종료하고 `127.0.0.1:8080`(백엔드)과 `127.0.0.1:3000`(프런트엔드)으로 전달하는 리버스 프록시를 두고, CLI를 공개 HTTPS URL로 연결하세요.
