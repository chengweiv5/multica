...
<Callout type="info">
**ポートはデフォルトで `127.0.0.1` でのみ待ち受けます。** `docker-compose.selfhost.yml` は公開されるすべてのポートをデフォルトで loopback にバインドします — `ss -tlnp` には `0.0.0.0:8080` は表示されず、他のマシンからはデフォルトでサービスにアクセスできません。信頼できる LAN 専用のテストでは `.env` に `MULTICA_BIND_ADDRESS=0.0.0.0` を設定できますが、サーバーのシークレットと Postgres の認証情報は、公開インターネット上に置いては絶対にいけません。公開またはマシン間アクセスが必要な場合は、TLS を終端するリバースプロキシをスタックの前に置いてください — [ステップ5b — マシン間: リバースプロキシを前に置く](#5b-cross-machine-front-with-a-reverse-proxy)を参照してください。
</Callout>
...
compose スタックはデフォルトで `127.0.0.1` で待ち受けるため、別のマシンにあるデーモンは `http://<server-ip>:8080` に直接接続できません — 通常は直接接続すべきでもありません。さもなければサーバーのシークレットが公開インターネットから到達可能になる可能性があります。信頼できる LAN 専用のテストでは `.env` に `MULTICA_BIND_ADDRESS=0.0.0.0` を設定してスタックを再起動できます。公開デプロイでは、TLS を終端し、`127.0.0.1:8080`（バックエンド）と `127.0.0.1:3000`（フロントエンド）へ転送するリバースプロキシをサーバーに置き、CLI を公開 HTTPS URL に向けてください。
