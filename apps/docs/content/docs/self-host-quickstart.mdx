...
<Callout type="info">
**Ports listen on `127.0.0.1` by default.** `docker-compose.selfhost.yml` binds every published port to loopback — `ss -tlnp` will not show `0.0.0.0:8080`, and the services are unreachable from other machines by default. For trusted LAN-only testing you can set `MULTICA_BIND_ADDRESS=0.0.0.0` in `.env`, but secrets and Postgres credentials must never sit on the open internet. For public or cross-machine access, front the stack with a reverse proxy that terminates TLS — see [Step 5b — Cross-machine: front with a reverse proxy](#5b-cross-machine-front-with-a-reverse-proxy).
</Callout>
...
Because the compose stack listens on `127.0.0.1` by default, a daemon on a different machine cannot reach `http://<server-ip>:8080` directly — and you usually do not want it to, since server secrets would otherwise be reachable from the open internet. For a trusted LAN-only test you can set `MULTICA_BIND_ADDRESS=0.0.0.0` in `.env` and restart the stack. For public deployments, put a reverse proxy on the server that terminates TLS and forwards to `127.0.0.1:8080` (backend) and `127.0.0.1:3000` (frontend), then point the CLI at the public HTTPS URL:
