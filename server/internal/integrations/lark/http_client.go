...
func outboundMessageRequest(chatID ChatID, receiveIDType, msgType, content string, target ReplyTarget) (string, map[string]any) {
	if target.IsSet() {
...
func (c *httpAPIClient) SendInteractiveCard(ctx context.Context, p SendCardParams) (string, error) {
	...
	path, body := outboundMessageRequest(p.ChatID, "chat_id", "interactive", p.CardJSON, p.ReplyTarget)
	var resp struct {
		Code int    `json:"code"`
		Msg  string `json:"msg"`
		Data struct {
...
func (c *httpAPIClient) SendTextMessage(ctx context.Context, p SendTextParams) (string, error) {
	...
	path, body := outboundMessageRequest(p.ChatID, p.ReceiveIDType, "text", string(contentBytes), p.ReplyTarget)
	var resp struct {
		Code int    `json:"code"`
		Msg  string `json:"msg"`
		Data struct {
...
func (c *httpAPIClient) SendMarkdownCard(ctx context.Context, p SendMarkdownCardParams) (string, error) {
	...
	path, body := outboundMessageRequest(p.ChatID, "chat_id", "interactive", string(cardBytes), p.ReplyTarget)
	var resp struct {
		Code int    `json:"code"`
		Msg  string `json:"msg"`
		Data struct {
