...
func TestHTTPClient_SendMarkdownCard_RewritesLocalImages(t *testing.T) {
	...
	fake.stubSend(
		...
		func(r *http.Request, body map[string]string) {
			...
			if strings.Contains(content, "![](/data00/home/yangchengwei.1/multica_workspaces/ws/workdir/lark-auth-qr.png)") {
				t.Fatalf("local image markdown must not be forwarded verbatim: %q", content)
			}
			if !strings.Contains(content, "[image omitted: /data00/home/yangchengwei.1/multica_workspaces/ws/workdir/lark-auth-qr.png]") {
				t.Fatalf("local image path should be preserved as text for debugging: %q", content)
			}
		},
	)
	...
	_, err := c.SendMarkdownCard(context.Background(), SendMarkdownCardParams{
		InstallationID: testCreds(),
		ChatID:         ChatID("oc_chat_42"),
		Markdown:       "Scan this:\n\n![](/data00/home/yangchengwei.1/multica_workspaces/ws/workdir/lark-auth-qr.png)",
	})
	if err != nil {
		t.Fatalf("send markdown card: %v", err)
	}
